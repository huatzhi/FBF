...
class WemaFactory {
  /**
   * Setup a factory that fill up WEMA values of certain dataSet
  ...
  async init() {
    ...
    if (!this.lastProcessedCandle) {
      this.wema = new WEMA({ period: this.period, values: [] });
      return;
    }
  ...
  async fillNext(batch = 100) {
    ...
    const bulkWriteQueries = bars.map((bar) => {
      const result = this.wema.nextValue(bar.close);
